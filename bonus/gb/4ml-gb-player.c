/*H*****************************************************************************
*  FILENAME:     4ml-gb-player.c
*  DESCRIPTION:  Mega Micro Music Macro Language (4ML) Player for Game Boy
*
*  NOTES:        To be compiled with GBDK 3.0.
*
*                Make sure you include exactly 4 channels worth of material.
*
*                Wasn't sure where else to put this, so it can go in here. It's
*                an adaptation of this: https://github.com/protodomemusic/mmml/
*                but using the 4ML compiler. Needed to write some long-form
*                Game Boy music for a project, so updated the player to use the
*                nicer 4ML syntax.
*
...
*                generator.
*
*                Limits the capabilities of the sound engine to those operations
*                available in μMML (see link above). Essentially, this
*                approximates 1-bit music by linking the pulse width to the
*                volume.
*
*                Wave channel generates a pulse wave only and is shifted up an
*                octave so that the range matches channels 1 and 2.
*
*                TO-DO:
*                - Align note->drum assignations with those defined in
*                  super-wavexe's wave-data.h.
*                - Support ties, panning, and instruments.
*
*  AUTHOR:       Blake 'PROTODOME' Troise
*  PLATFORM:     Command Line Application
*  DATE:         12th May 2020 (Original)
...
INT8   channel_volume   [TOTAL_VOICES];
...
void main()
{
	...
	{
		data_pointer[i]  = _4ml_data[(i*2)];
		data_pointer[i]  = data_pointer[i] << 8;
		data_pointer[i]  = data_pointer[i] | _4ml_data[(i*2)+1];
		transposition[i] = 0;
		...
		if(tick_counter-- == 0)
		{
			...
			for(voice = 0; voice < TOTAL_VOICES; voice++)
			{
				...
				if(length[voice] == 0)
				{
					...
					buffer1 = _4ml_data[(data_pointer[voice])] >> 4;
					// second nibble of data
					buffer2 = _4ml_data[data_pointer[voice]] & 0x0F;

					// function command
					if(buffer1 == 15)
					{
						// another buffer for commands that require an additional byte.
						buffer3 = _4ml_data[data_pointer[voice] + 1];
						...
						else if(buffer2 == 2)
						{
							...
							data_pointer[voice] = _4ml_data[(buffer3 + TOTAL_VOICES) * 2];
							data_pointer[voice] = data_pointer[voice] << 8;
							data_pointer[voice] = data_pointer[voice] | _4ml_data[((buffer3 + TOTAL_VOICES) * 2) + 1];
						}
						...
						{
							// 4ml scales everything to signed bytes (confused me for a bit)
							transposition[voice] = buffer3 - 100;
							data_pointer[voice] += 2;
						}
						...
						else if(buffer2 == 14)
						{
							...
							if (buffer3 == 0 || buffer3 == 1)
							{
								...
								if (voice == 0)
								{
									...
									// check for overflows
									if (channel_volume[0] > 15)
										channel_volume[0] = 15;
									else if (channel_volume[0] < 1)
										channel_volume[0] = 1;

									current_volume = channel_volume[0];

									// change pulse width w/ volume
									if (current_volume > 10)
										NR11_REG = 0x80;
									else if (current_volume > 4 && current_volume <= 10)
										NR11_REG = 0x40;
									else if (current_volume <= 4)
										NR11_REG = 0x00;
								}
								if (voice == 1)
								{
									...
									// check for overflows
									if (channel_volume[1] > 16)
										channel_volume[1] = 16;
									else if (channel_volume[1] < 1)
										channel_volume[1] = 1;

									current_volume = channel_volume[1];
								...
								if (voice == 2)
								{
									...
									// check for overflows
									if (channel_volume[2] > 16)
										channel_volume[2] = 16;
									else if (channel_volume[2] < 1)
										channel_volume[2] = 1;

									current_volume = channel_volume[2];

									update_wavetable(current_volume);
								}
							}
						...
						else if(buffer2 == 15)
						{
							...
							{
								data_pointer[voice] = _4ml_data[(voice * 2)];
								data_pointer[voice] = data_pointer[voice] << 8;
								data_pointer[voice] = data_pointer[voice] | _4ml_data[(voice*2)+1];
							}
						}
					...
					if(buffer1 != 0 && buffer1 < 14)
					{
						...
						else
						{
							...
							{
								case 1: // perc 1
									update_channel_d(0x04, 0xF, 0x02, 0x01, 0x00);
									drum_duration = 1;
									break;
								case 2: // perc 2
									update_channel_d(0x01, 0xF, 0x01, 0x01, 0x00);
									drum_duration = 200;
									break;
								case 3: // kick
									update_channel_d(0x04, 0xF, 0x04, 0x01, 0x00);
									drum_duration = 6;
									break;
								case 4: // snare
									update_channel_d(0x04, 0xF, 0x02, 0x01, 0x00);
									...
									break;
								case 5:  // hh
									update_channel_d(0x01, 0xB, 0x01, 0x01, 0x00);
									drum_duration = 6;
									break;
							}
						}
					...
					{
						// rather than silencing the channel, re-triggering the
